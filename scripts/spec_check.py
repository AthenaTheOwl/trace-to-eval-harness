...
def main() -> int:
    ...
    expected = {f"R-TTE-{index:03d}" for index in range(1, 15)} | {
        f"R-TTE-SCHEMA-{index:03d}" for index in range(1, 6)
    }
    if req_ids != expected:
        violations.append(
            "requirements must define R-TTE-001..014 and "
    ...
    if violations:
        return report(violations)
    print("spec_check OK (19 requirements, owner roles, DEC coverage)")
    return 0
